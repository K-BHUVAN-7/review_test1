...
@Component({
  selector: 'app-edit-manager-tickets',
  standalone: true,
  imports: [SharedModule],
  templateUrl: './edit-manager-tickets.component.html',
  styleUrl: './edit-manager-tickets.component.scss'
})
export class EditManagerTicketsComponent {
  ...
  managerIssueForm: FormGroup = new FormGroup({});
  editData:any = {};
  userDetails: any = {};
      ...
      next: (res: any) => {
        ...
        if (res.propertyList?.status == 'ok') {
          ...
          if (this.userDetails?.userType == 'admin') {
            ...
            this.masterList['propertyList'] = _.filter(res.propertyList.data, (e) =>
            
              _.includes(this.userDetails.propertyIds, e._id)
  ...
  loadForm() {
    ...
    this.managerIssueForm = this.fb.group({
      ...
      'tenantName': [ this.editData.tenantName?._id || null, Validators.required ],
    ...
    if (this.mode == 'Update') {
      ...
      let selectedIssueId = this.editData?.issueName?._id;
      ...
      let selectedUnitId = this.editData?.unitName?._id;

      if (selectedUnitId) {

        this.filteredTenantList = _.filter(this.masterList['tenantList'], (tenant: any) =>
        
          tenant?.unitName?._id == selectedUnitId
        
        );

        let selectedTenantId = this.editData?.tenantName?._id;
        
        let tenantExists = _.some(this.filteredTenantList, { _id: selectedTenantId });

        this.managerIssueForm.patchValue({
        
          'tenantName': tenantExists ? selectedTenantId : null
    ...
    this.managerIssueForm.get('unitName')?.valueChanges.subscribe((unitId: string) => {

      this.filteredTenantList = _.filter(this.masterList.tenantList, (tenant: any) => {
      
        return tenant?.unitName?._id == unitId;
      
      });

      this.managerIssueForm.patchValue({ 'tenantName': null });
  ...
  issueSelected(issueId: string, preselectedStaffId: string = ''): void {
    ...
    let issue = _.find(this.masterList['issueList'], { _id: issueId });
    ...
    let filteredStaff = _.filter(this.masterList['staffList'], (staff: any) =>
  ...
  get f(): any { return this.managerIssueForm.controls; }

  get images(): FormArray {return this.managerIssueForm.get('images') as FormArray}
  ...
  onSubmit(){
    ...
    if(this.managerIssueForm?.invalid) return;
  
    let payload = this.managerIssueForm.getRawValue();
    ...
    Object.keys(this.managerIssueForm.controls).forEach((key) => {

      if (key == 'images') {

        this.managerIssueForm.value.images.forEach((item: any) => {
