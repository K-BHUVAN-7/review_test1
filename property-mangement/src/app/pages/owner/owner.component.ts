...
export class OwnerComponent {
  ...
  openModal(data?:any){
    ...
    if(!_.isEmpty(queryParams)) {
      ...
      this.service.navigate({ 'url': 'app/owner/create-owner', queryParams });

    } else {

      this.service.navigate({ 'url': 'app/owner/create-owner' }); 
  ...
  getOwnerList(){
    ...
    let payload = { 'parentCompanyId': this.userDetails.parentCompanyId, 'companyId': this.companyId, 'branchId': this.branchId, 'userType': 'owner', ...filterValues }

    this.service.postService({'url': '/otherUser/list', payload, params }).subscribe((res: any) => {

      if(res.status == 'ok') {

        this.masterList['ownerList'] = res.data;
  ...
  inactiveOwner(data:any) {

    this.confirmationDialog.confirm({ 'message': `Do you want to ${data.is_active ? 'Inactive' : 'Active'} Owner ?`, 'type': 'info', title : data.is_active ? 'Inactive' : 'Active' }).then((confirmed: any) => {
  ...
  xlDownload(){
    ...
    let params =  { 'parentCompanyId': this.userDetails.parentCompanyId, 'companyId': this.companyId, 'branchId': this.branchId, 'userType': 'owner'  };

    this.service.getFile({ "url": "/getOwnerExcel", params }).subscribe((res: any) => {
