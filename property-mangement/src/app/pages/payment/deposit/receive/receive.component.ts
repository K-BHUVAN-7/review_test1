...
export class ReceiveComponent {
  ...
  loadForm() {
    ...
    this.receiveForm = this.fb.group({
      ...
      'depositAmount': [ this.editData?.amountOutstanding || '', [Validators.required] ],
      
      'paymentDate': [ this.editData?.paymentDate || '',  [Validators.required] ],
      
      'amountPaid': [ 0, [Validators.required, this.amountPaidValidator()] ],
    ...
    this.setPayValidater(this.receiveForm.get('paymentMode')?.value);

    this.receiveForm.get('paymentMode')?.valueChanges.subscribe((mode) => {
  ...
  get f(): any { return this.receiveForm.controls; }
  ...
  submit() {
    ...
    let payload = this.receiveForm.getRawValue();
