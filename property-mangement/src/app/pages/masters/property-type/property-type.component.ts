...
export class PropertyTypeComponent {
  ...
  @ViewChild("propertyTypeModel") propertyTypeModel!: ModalComponent;
  ...
  ngOnInit(): void {
    ...
    this.permissions = this.service.getPermissions({ pathArr: ["Masters", "Property Type"], isNeedBranchList: true, 'permission': ['create', 'view', 'edit', 'delete']});
  ...
  getPropertyTypeList(){
    ...
    this.service.postService({ "url": "/master/propertyType/list", payload, params }).subscribe((res: any) => {

      if(res.status == 'ok') {

        this.masterList['propertyTypeList'] = res.data;
  ...
  get f():any { return this.propertyTypeForm.controls; }
  ...
  submit() {
    ...
    if(this.propertyTypeForm.invalid) return;
  
    let payload = this.propertyTypeForm.value;
    ...
    this.confirmationDialog.confirm({
      ...
      if(confrimation){
        ...
        forkJoin({
          ...
          this.service.postService({ 'url': '/master/propertyType/create', payload }) : 
        
          this.service.patchService({ 'url': `/master/propertyType/${this.editData?._id}`, payload })
  ...
  deletePropertyType(data:any) {
    ...
    this.confirmationDialog.confirm({ 'message': 'Do you want to Delete?', 'type': 'error', title: 'Delete' }).then((confirmed: any) => {
      ...
      if(confirmed) {
        ...
        this.service.deleteService( "/master/propertyType/"+ data?._id).subscribe((res: any) => {
  ...
  xlDownload(){
    ...
    this.service.getFile({ "url": "/master/getPropertyTypeExcel", params }).subscribe((res: any) => {
      ...
      a.download = 'Property Type List.xlsx';
