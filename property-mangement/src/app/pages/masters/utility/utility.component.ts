...
export class UtilityComponent {
  ...
  @ViewChild("utilityModal") utilityModal!: ModalComponent;
  ...
  ngOnInit(): void {
    ...
    // if(_.includes(this.permissions,'view')){
  ...
  getUtilityList(){
    ...
    this.service.postService({ "url": "/master/utility/list", payload, params }).subscribe((res: any) => {

      if(res.status == 'ok') {

        this.masterList['utilityList'] = res.data;
  ...
  loadForm() {
    ...
    this.utilityForm = this.fb.group({
      ...
      'utilityName': [ this.editData.utilityName || "", Validators.required ] ,

      'defaultAmt': [ this.editData.defaultAmt || 0 ],
      
      'paidBy': [  this.mode == 'Update' ? (this.editData.paidBy == 'owner' ? false : true) : true ],

      'utilityAmtType': [  this.mode == 'Update' ? (this.editData.utilityAmtType == 'fixed' ? false : true) : true ],

    })

  }

  get f():any { return this.utilityForm.controls; }
  ...
  submit() {
    ...
    let payload = this.utilityForm.value;

    payload['utilityAmtType'] = payload?.utilityAmtType == true ? 'variable' : 'fixed';

    payload['paidBy'] = payload?.paidBy == true ? 'tenant' : 'owner'
    ...
    this.confirmationDialog.confirm({
      ...
      if(confrimation){
        ...
        forkJoin({
          ...
          this.service.postService({ 'url': '/master/utility/create', payload }) : 
        
          this.service.patchService({ 'url': `/master/utility/${this.editData?._id}`, payload })
  ...
  deletePropertyType(data:any) {
    ...
    this.confirmationDialog.confirm({ 'message': 'Do you want to Delete?', 'type': 'error' , title : 'Delete' }).then((confirmed: any) => {
      ...
      if(confirmed) {
        ...
        this.service.deleteService( "/master/utility/"+ data?._id).subscribe((res: any) => {
  ...
  xlDownload(){
    ...
    this.service.getFile({ "url": "/master/getUtilityExcel", params }).subscribe((res: any) => {
